...
export class MembersController {
    ...
    async removeFriend(req, res, next) {
        try {
            ...
            loggedMember.friends = loggedMember.friends.filter((item) => item.id !== memberToRemove.id);
            memberToRemove.friends = memberToRemove.friends.filter((item) => item.id !== loggedMember.id);
            const memberUpdated = await this.repo.update(loggedMember);
    ...
    async removeEnemy(req, res, next) {
        try {
            ...
            loggedMember.enemies = loggedMember.enemies.filter((item) => item.id !== memberToRemove.id);
            memberToRemove.enemies = memberToRemove.enemies.filter((item) => item.id !== loggedMember.id);
